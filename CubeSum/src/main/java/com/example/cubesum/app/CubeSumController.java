...
public class CubeSumController {
	...
	StoreData operation(@RequestBody Operation opt) {
		return repositoryData.findById(1L).map(storeData -> {
			...
			int test = storeData.getNumberTest();
			int points = storeData.getNumberPoints();
			int operations = storeData.getNumberOperations();
			if (test > 0) {
				if (operations > 0) {
                	...
                	storeData.setResponse("Define new points and operations");
				}
			} else {
				storeData.setResponse("Not have test to do!");
			}
			
			if (success) {
				storeData.setNumberTest(test - 1);
				storeData.setNumberOperations(operations - 1);
			}
			repositoryData.save(storeData);
